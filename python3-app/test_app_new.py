...
# --- импорт вашего приложения: main.py или app.py ---
import main as myapp  # если файл называется main.py


def _write_templates(root_dir: str):
    """
    Ваш код рендерит 'templates/xxx.html' (с префиксом templates/ в имени).
    Поэтому template_path=root_dir, а файлы кладём в root_dir/templates/.
    """
...
def allocate_id_no_eval(redis_client, key: str) -> str:
    """
    Замена allocate_id() для fakeredis, т.к. fakeredis может не уметь EVAL.
    Семантика совпадает:
      - вернуть текущее значение autoID
      - затем увеличить autoID на 1
    """
    v = redis_client.get(key)
    if v is None:
        redis_client.set(key, 1)
        v = b"1"
    current = v.decode() if isinstance(v, (bytes, bytearray)) else str(v)
...
class TestApp(AsyncHTTPTestCase):
    def setUp(self):
        # 1) Подменяем redis на fakeredis
        self.fake_redis = fakeredis.FakeStrictRedis()
        myapp.r = self.fake_redis

        # 2) Подменяем allocate_id на версию без EVAL
        # (критично для прохождения тестов)
        myapp.allocate_id = allocate_id_no_eval
    ...
    def get_app(self):
        ...
        return tornado.web.Application(
            ...
            template_path=self._tmpdir.name,
            autoreload=False,
            debug=False,
    ...
    def test_hospital_post_requires_name_and_address(self):
        resp = self._post_form("/hospital", {"name": "", "address": ""})
    ...
    def test_redis_connection_error_returns_400(self):
        # Теперь GET /hospital читает через SCAN (list_entities -> scan_iter),
        # поэтому ломаем именно scan_iter.
        def boom(*args, **kwargs):
            raise redis.exceptions.ConnectionError("nope")

        myapp.r.scan_iter = boom
